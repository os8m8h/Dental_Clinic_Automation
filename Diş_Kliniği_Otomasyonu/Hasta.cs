...
namespace Diş_Kliniği_Otomasyonu
{
    public partial class Hasta : Form
    {
        ...
        {
            string query = "insert into HastaTbl values('"+HAdSoyadTb.Text+"','"+HTelefonTb.Text+"','"+AdresTb.Text+"','"+HDogumTarih.Value.Date+"','"+HCinsiyetCb.SelectedItem.ToString()+"','"+AlerjiTb.Text+"')";
        ...
        private void guna2GradientButton1_Click(object sender, EventArgs e)
        {
            ...
            if(key==0)
            {
                MessageBox.Show("Silinecek Hastayı Seçeniz");
            }
            else
            {
                try
        ...
        private void guna2GradientButton2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    string query = "Update HastaTbl set HAd='"+HAdSoyadTb.Text+"',HTelefon='"+HTelefonTb.Text+"',HAdres='"+AdresTb.Text+"',HTarih='"+HDogumTarih.Text+"',HCinsiyet='"+HCinsiyetCb.SelectedItem.ToString()+"',HAlerji='"+AlerjiTb.Text+"' where HId=" + key + ";";
                    Hs.HastaSil(query);
